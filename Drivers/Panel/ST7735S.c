					...
					GPIO_TypeDef *RS_PORT, uint16_t RS_PIN )
{
	...
	spi_set_prescaler( SPI_BAUDRATEPRESCALER_16 ); // 108M / 8 = 13.5MBits/s
...
{
	sx = ( sx < ResolutionWidth ) ? sx : (ResolutionWidth - 1);
	sy = ( sy < ResolutionHeight ) ? sy : (ResolutionHeight - 1);
	ex = ( ex < ResolutionWidth ) ? ex : (ResolutionWidth - 1);
	ey = ( ey < ResolutionHeight ) ? ey : (ResolutionHeight - 1);
